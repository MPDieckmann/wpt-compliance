...
for (let i = 0; i < args.length; i++) {
  const arg = args[i];
  if (arg === "-d" || arg === "--debug") flags.debug = true;
...
async function main() {
  ...
  if (command === "list") {
    ...
    if (features.length === 0) {
      console.log(
        "No cached features. Use `wpt fetch <feature>` to download tests.",
      );
    } else {
      console.log("Cached WPT features:\n");
      for (const f of features) {
        console.log(
          `  • ${f.feature}  (${f.testFiles} test files, ${f.files} total, fetched ${f.fetchedAt})`,
        );
      }
      console.log("");
    }
  ...
  if (command === "run") {
    ...
    if (!feature) {
      console.error(
        "Error: Missing feature name. Usage: wpt run <feature> [files..]",
      );
      process.exit(1);
    }
